...
const ClientComponent: React.FC<ClientComponentProps> = ({
    ...
    return (
        <div className="flex h-screen overflow-scroll">
            ...
            {/* Main content */}
            <div className="flex-1 p-4 bg-slate-100  text-black">
                {sectionId === "account-preferences" && (
                    <AccountPreferences user={user} />
                )}
                {sectionId === "sign-in-security" && (
                    <SignInSecurity user={user} />
                )}
                {sectionId === "update-image" && <UpdateImage user={user} />}
